...
ReminderWidget::ReminderWidget(QWidget *parent)
    : QWidget{parent}
{
    qDebug() << "in reminder constructor ";
    ...
    this->setObjectName("reminderWidget");
    this->setAttribute(Qt::WA_DeleteOnClose);
    netMgrObj =  MyNetworkManager::instance();

    m_reminderLayout = new QVBoxLayout();
    m_reminderLayout->setContentsMargins(30,20,30,20);
    ...
    project_label = new QLabel("Work");
    project_label->setFont(QFont("Ubuntu",12,500));
    task_title_label = new QLabel("Task Title");

    d_taskActiveTimeInReminder = new QLabel("00:00:00");
    d_taskActiveTimeInReminder->setFixedSize(95,27);
    d_taskActiveTimeInReminder->setAlignment(Qt::AlignCenter);
    ...
    d_taskReminderTimeLabel->setFixedSize(95,27);
    d_taskReminderTimeLabel->setAlignment(Qt::AlignCenter);
   ...
   d_taskReminderTimeLabel->setStyleSheet(
    ...
    custom_time_button->setCheckable(true);
...
//    custom_time_label->setAutoFillBackground(true);
    ...
    work_tasktitle_hor_layout->addWidget(project_label);
    ...
    custom_time_layout->setContentsMargins(0,18,10,2);
    ...
    custom_widget->setFixedSize(348,115);
    custom_widget->move(30,240);
...
//         );
    ...
    m_reminderLayout->setSpacing(11);

    // set_reminder_button->setVisible(false);
    // update_widget->setVisible(true);
    this->setLayout(m_reminderLayout);
    // this->show();
    reminderTime = QTime(0,0,0);

    // signals and slots
    connect(custom_time_button,&QPushButton::clicked,this,&ReminderWidget::onCustomBtnClicked);
    ...
    connect(done_button,&QPushButton::clicked,this,[=]{
        ...
        if(input_time != 0){
            ...
            if(time_combo_box->currentText() == "mins"){
                if(input_time < 60){
                    reminderTime = QTime(0,input_time,0);
                }else if(input_time > 60){
                    reminderTime = QTime(input_time/60,input_time%60,0);
                }else
                    reminderTime = QTime(1,0,0);
            }
            else{
                if(input_time < 24)
                    reminderTime = QTime(input_time,0,0);
            }
            qDebug() << "input time " << reminderTime.toString("hh:mm:ss");
            input_time_lineedit->clear();
            input_time = 0;
        }
        custom_widget->setVisible(false);
    ...
    connect(set_reminder_button,&QPushButton::clicked,this,[=]{
        ...
        qDebug() << "remin value " << reminderTime;
        // custom_time_button->setText("Custom Time");
        // time_combo_box->setCurrentText("mins");
        // d_taskReminderTimeLabel->setText(reminderTime.toString("hh:mm:ss"));
        // this->hide();
        // set_reminder_button->setVisible(false);
        // if(lastClickedIndex <=5){
        //     // buttons[lastClickedIndex]->setStyleSheet("");
        ...
        if(reminderTime != QTime(0,0,0)){
            qDebug() << "remin value " << reminderTime;
            // emit displayReminderTime(reminderTime);
            emit updateReminderTimeSignal(reminderTime);
        }
        else{
            QMessageBox msgBox;
            msgBox.setWindowTitle("Complete Task");
            msgBox.setText("Reminder Time is not valid...!");
...
void ReminderWidget::addbuttonbackground(int index)
{
    ...
    if(timetext.contains(" min")){
        ...
        reminderTime = QTime(0,button_time,0);
    }
    else{
        timetext.remove(" hr");
        button_time = timetext.toInt(&ok);
        reminderTime = QTime(button_time,0,0);
    }
    qDebug() << reminderTime.toString("hh:mm:ss");
...
void ReminderWidget::onCustomBtnClicked(bool isClicked)
{
    qDebug() << "custom button clicked " << isClicked;
    // if (lastClickedIndex <=5) {
    //     buttons[lastClickedIndex]->setStyleSheet(
    //         "QPushButton {"
    //         "background-color: rgba(35, 31, 32, 0.1);"
